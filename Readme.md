...
# Fire B.O.S.S. (BaseX-Oriented Storage System)
Fire B.O.S.S. is an implementation of a FHIR conformant Server, that sits in front of a BaseX database and performs all operations with XQuery. This allows for the server to be used with FHIR dependent subsystems while making it possible for the user to perform powerful queries on the underlying BaseX database.
...
**Important:** Fire B.O.S.S. itself lacks security features, there is no implementation of authentication protocols built in. Do not use the server in its current form with sensitive patient data, unless you are able to ensure that no unauthorized user has access to your server.
...
Fire B.O.S.S is carefully modeled to conform to the [FHIR RESTful API](http://hl7.org/fhir/http.html) and the [FHIR Search](http://hl7.org/fhir/search.html) specifications.
